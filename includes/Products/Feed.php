...
class Feed {
	...
	public function regenerate_feed() {
		...
		if ( facebook_for_woocommerce()->get_integration()->is_new_style_feed_generation_enabled() ) {
			facebook_for_woocommerce()->dev_log( 'using new style feed generation' );
			$generate_feed_job = facebook_for_woocommerce()->job_manager->generate_product_feed_job;
			$generate_feed_job->queue_start();
		} else {
			facebook_for_woocommerce()->dev_log( 'using old style feed generation' );
			$feed_handler = new \WC_Facebook_Product_Feed();
			$feed_handler->generate_feed();
		}
	}
	...
	public function schedule_feed_generation() {
		facebook_for_woocommerce()->dev_log( 'trying to schedule product feed generation' );
		...
		if ( ! $store_allows_sync || ! $store_allows_feed ) {
			facebook_for_woocommerce()->dev_log( 'couldnt schedule product feed generation. ' . 'is store configured: ' . ( $configured_ok ? 'yes' : 'no' ) . ' store allows sync: ' . ( $integration->is_product_sync_enabled() ? 'yes' : 'no' ) . ' store allows feed: ' . ( $integration->is_legacy_feed_file_generation_enabled() ? 'yes' : 'no' ) );
		...
		if ( ! as_next_scheduled_action( self::GENERATE_FEED_ACTION ) ) {
			facebook_for_woocommerce()->dev_log( 'scheduling product feed generation' );
	...
	public function send_request_to_upload_feed() {
		...
		if ( empty( $feed_id ) ) {
			facebook_for_woocommerce()->dev_log( ' product Feed: integration feed ID is null or empty, feed will not be uploaded' );
		...
		try {
			facebook_for_woocommerce()->dev_log( 'sending request to upload product feed, feed data url: ' . self::get_feed_data_url() );
	...
	public function retrieve_or_create_integration_feed_id() {
		facebook_for_woocommerce()->dev_log( 'trying to retrieve or create feed id' );
